(function ($) {
    ...
    var loader, loaderOv, loadAn; function pageLoader() {
        loader = $('.alioth-page-loader'); if (siteLoader == true) {
            $('.apl-count').wrap('<div class="apl-wrapper"></div>'); var loaderLayout = loader.data('layout'); loader.addClass(loaderLayout)
            ...
            if (siteHeader.hasClass('classic_menu')) {
                ...
                loadAn.fromTo('.main-menu > li > a', 1, { y: '100%' }, { y: '0%', stagger: .1, ease: 'power2.out', onComplete: function () { gsap.set('.main-menu > li', { clearProps: 'all' }) } }, 3)
            } else { loadAn.to('.toggle-line', 1, { width: 50, ease: 'power2.out', stagger: .3 }, 3) }
    ...
    function mouseCursor() {
        if (pageCursor == true) {
            let mouseCursor = $('#mouseCursor'), circle = $('#cursor'), dot = $('#dot'), cursorActive; gsap.set(mouseCursor, { xPercent: -50, yPercent: -50 }); let ball = mouseCursor
            let pos = { x: window.innerWidth / 2, y: window.innerHeight / 2 }; let mouse = { x: pos.x, y: pos.y }; let speed = 0.1; cursorActive = true; let xSet = gsap.quickSetter(ball, "x", "px", "force3d"); let ySet = gsap.quickSetter(ball, "y", "px", "force3d"); window.addEventListener("mousemove", e => { mouse.x = e.x; mouse.y = e.y; }); gsap.ticker.add(() => { if (cursorActive) { let dt = 1.0 - Math.pow(1.0 - speed, gsap.ticker.deltaRatio()); pos.x += (mouse.x - pos.x) * dt; pos.y += (mouse.y - pos.y) * dt; xSet(pos.x); ySet(pos.y); } }); function cursorHovers() {
                var darkCircle = mouseCursor.data('dark-circle'), darkDot = mouseCursor.data('dark-dot'), lightCircle = mouseCursor.data('light-circle'), lightDot = mouseCursor.data('light-dot'), curBg, iconColor; if ($('body').hasClass('dark')) {
                    gsap.set(cursor, { borderColor: lightCircle })
                    gsap.set(dot, { background: lightDot })
                    curBg = lightCircle; iconColor = lightDot;
                } else {
                ...
                $('.section').on('mouseenter', function () {
                    let $this = $(this), color = $this.css('background-color'), hsl = gsap.utils.splitColor(color, true), lightness = hsl[hsl.length - 1]; if ((lightness < 50) && (lightness != 0)) {
                ...
                var projectHovers = $('.ar-work, .fw-project, .cs-title, .sl-project, .wall-project, .alioth-single-project, .aw-project'); projectHovers.on('mouseenter', function (e) {
                ...
                var borderHovers = $('.menu-toggle, .fs-prev, .fs-next, .ss1-dots, .ss2-dot, .ss2-prev, .ss2-next, .ss1-prev, .ss1-next, .a-plus-button, .scroll-notice, .a-test-next, .a-test-prev, .cart-button, .cpq-reduce, .cpq-increase, .swiper-pagination-bullet'); borderHovers.on('mouseenter', function (e) {
                    gsap.to(mouseCursor, { width: 100, height: 100 })
                ...
                var dotHovers = $('ul.main-menu a, .a-button, .a-client, .fs-button, .alioth-latest-posts .post, .field-wrap'); dotHovers.on('mouseenter', function (e) {
                    gsap.to(mouseCursor, { width: 100, height: 100 })
                    gsap.to(cursor, { backgroundColor: curBg, borderWidth: 0 })
                    gsap.to(dot, { opacity: 0 })
                })
                ...
                var imageHovers = $('.single-image.lightbox'); imageHovers.on('mouseenter', function (e) {
                ...
                imageHovers.on('mouseleave', function (e) {
                    gsap.to('#cursorIcon', { scale: 0, onComplete: function () { $('#cursorIcon').remove(); } })
                    gsap.to(mouseCursor, { width: 50, height: 50 })
                    gsap.to(dot, { opacity: 1 })
                })
            }
            cursorHovers(); let cursorLoading; barba.hooks.before((data) => {
                cursorLoading = gsap.timeline({ overwrite: true }); cursorLoading.to(mouseCursor, .3, { width: 50, height: 50, }, 0)
                ...
                }); cursorLoading.pause(); cursorLoaded.to(dot, .8, { top: '50%', left: '50%', ease: 'power2.inOut' }, .4)
            });
        } else { $('#mouseCursor').hide(); }
    }
    function siteHeaderSet() {
        siteHeader = $('.site-header'); gsap.set(siteHeader, { clearProps: 'all' })
        var siteNav = $('.site-navigation'), headerHeight = siteHeader.outerHeight(), siteContent = $('#page'), headerWrapper = $('.header-wrapper'); let mobileQuery = window.matchMedia('(max-width: 900px)'); if (mobileQuery.matches) { siteHeader.addClass('fullscreen_menu'); siteNav.addClass('fullscreen') } else {
            if ((menuStyle === 'classic') || (menuStyle == null)) {
                siteHeader.addClass('classic_menu')
                siteNav.addClass('classic')
            } else if (menuStyle === 'overlay') { siteHeader.addClass('fullscreen_menu'); siteNav.addClass('fullscreen') }
        }
        ...
        if (siteHeader.hasClass('sticky_header')) {
            var showcaseScroll = ScrollTrigger.getById('showcaseScroll'), stickyStart = 500, stickyBg = '#fff'; if (siteHeader.hasClass('light')) { var stickyBg = "rgba(19,19,19,1)" }
            if (showcaseScroll) {
                var stickyStart = showcaseScroll.end; gsap.set(siteHeader, { position: 'fixed' })
                var sitkcyHdr = ScrollTrigger.create({
                    trigger: siteHeader, start: stickyStart + 'bottom', id: 'stickyHeader', markers: false, onLeaveBack: function () {
                        gsap.to(siteHeader, { position: 'fixed', y: '0%', height: 150, duration: .75, backgroundColor: 'transparent', delay: .5, ease: 'power2.out' })
                        gsap.to(headerWrapper, { top: '40%', delay: .5, duration: .75, ease: 'power2.out' })
                    }, onEnter: function () {
                        gsap.set(siteHeader, { position: 'absolute', top: stickyStart })
                        ScrollTrigger.create({
                            trigger: 'body', markers: false, start: stickyStart + 500 + 'top', end: 'bottom bottom', onEnter: function () {
                                gsap.set(siteHeader, { position: 'fixed', top: 0, y: '-100%', height: 100, backgroundColor: stickyBg, })
                                gsap.set(headerWrapper, { top: '40%' })
                            }, onUpdate: function (self, direction, progress) { if (self.direction == -1) { gsap.to(siteHeader, { y: '0%', }) } else { gsap.to(siteHeader, { y: '-100%', }) } }, onLeaveBack: function (self) { self.kill(); }
                        })
                    }, onEnterBack: function () { gsap.set(siteHeader, { position: 'fixed', top: 0 }) }
                })
            } else {
                gsap.set(siteHeader, { position: 'absolute' })
                ScrollTrigger.create({
                    trigger: 'body', start: 'top+=500 top', end: 'bottom bottom', id: 'stickyHeader', markers: false, onEnter: function () {
                        if (pageSet.hasClass('dark')) { var curBg = 'rgba(19,19,19,1)' } else { var curBg = '#ebebeb' }
                        ...
                        ScrollTrigger.create({
                            trigger: 'body', markers: false, start: 'top top', end: 'bottom bottom', onUpdate: function (self, direction, progress) { if (self.direction == -1) { gsap.to(siteHeader, { y: '0%', }) } else { gsap.to(siteHeader, { y: '-100%', }) } }, onLeaveBack: function (self) {
                                self.kill(); gsap.to(siteHeader, { position: 'absolute', y: '0%', duration: .75, backgroundColor: 'transparent', delay: .5, clearProps: 'all', ease: 'power2.out' })
    ...
    var resizeTimer; function classicNavigation() {
        var siteNav = $('.site-navigation'), menu = siteNav.children('.menu'), menuItem = menu.children('li'); if (siteNav.hasClass('classic')) {
            menuItem.each(function () {
                let $this = $(this), menuItemA = $this.children('a'); var classicSplit = new SplitText(menuItemA, { tyoe: 'chars', charsClass: 'menu-tit-char' }), mobileQuery = window.matchMedia('(max-width: 900px)')
    ...
    function mobileMenu() {
        var mobileQuery = window.matchMedia('(max-width: 900px)'), desktopQuery = window.matchMedia('(min-width: 900px)'); $(window).on('resize', function (e) {
            var siteNav = $('.site-navigation'); if (siteNav.hasClass('classic')) { if (mobileQuery.matches) { clearTimeout(resizeTimer); resizeTimer = setTimeout(function () { siteHeader.removeClass('classic_menu'); siteNav.removeClass('classic'); siteHeader.addClass('fullscreen_menu'); siteNav.addClass('fullscreen'); siteNav.addClass('desktop-classic'); fullscreenNavigation(); }, 250); } }
            if ((siteNav.hasClass('desktop-classic')) && (desktopQuery.matches)) {
                clearTimeout(resizeTimer); resizeTimer = setTimeout(function () {
                    $(".site-navigation > .fs-menu-wrapper").contents().unwrap(); siteHeader.addClass('classic_menu'); siteNav.addClass('classic'); siteHeader.removeClass('fullscreen_menu'); siteNav.removeClass('fullscreen'); siteNav.removeClass('desktop-classic'); gsap.set('.menu-item a', { clearProps: 'all' })
                    classicNavigation();
                }, 250);
            }
        });
    }
    mobileMenu(); function fullscreenNavigation() {
        if ($('.site-navigation').hasClass('fullscreen')) {
            var siteNav = $('.site-navigation'), menuItemHasSub = $('.menu-item.has-children > a'), subMenu = $('.site-navigation .menu li .sub-menu'), headerWrapper = $('.header-wrapper'), mainMenu = $('.main-menu'); siteHeader.addClass('menu_' + menuLayout)
            siteNav.wrapInner("<div class='fs-menu-wrapper'></div>")
            var menuWrapper = $('.fs-menu-wrapper'); menuItemHasSub.each(function () { let $this = $(this); $this.append('<span class="sub-toggle"><span class="sub-togg-line"></span><span class="sub-togg-line"></span></span>'); }); menuItemHasSub.on('click', function () {
                $('.sub-back').addClass('is-active'); var $this = $(this); let parentLi = $this.parent('li'), currentMenu = parentLi.parent('ul'), menuItemLi = currentMenu.children('li'), menuItemA = menuItemLi.children('a'); var menuOut = gsap.fromTo(menuItemA, { translateY: '0%', }, { translateY: '-100%', stagger: 0.03, duration: .4, ease: "power2.in", overwrite: true, onComplete: function () { currentMenu.addClass('hidden'); currentMenu.removeClass('opened'); $('.sub-back').addClass('is-active') } }); let subMenu = parentLi.children('ul'); let subMenuLi = subMenu.children('li'); let subMenuLiA = subMenuLi.children('a')
                var subAnim = gsap.fromTo(subMenuLiA, { translateY: "100%", }, { translateY: "0%", delay: .4, stagger: .05, overwrite: true, ease: "power2.out", onStart: function () { subMenu.addClass('opened') }, });
            }); $('.sub-back').on('click', function () {
                let currentMenu = $('.sub-menu.opened'), currentMenuLi = currentMenu.children('li'), currentMenuA = currentMenuLi.children('a'); gsap.fromTo(currentMenuA, { translateY: "0%", }, {
                    translateY: "100%", stagger: -0.05, overwrite: true, ease: "power2.in", onComplete: function () {
                        currentMenu.removeClass('opened')
                        currentMenu.addClass('hidden')
                    }
                })
                let parentMenu = currentMenu.parent('li').parent('ul')
                let parentMenuA = parentMenu.children('li').children('a'); gsap.fromTo(parentMenuA, { translateY: "-100%", }, { translateY: "0%", delay: .4, stagger: -0.05, overwrite: true, ease: "power2.out", onStart: function () { parentMenu.removeClass('hidden'); parentMenu.addClass('opened'); if ($('.main-menu').hasClass('opened')) { $('.sub-back').removeClass('is-active'); } } })
            }); var menuItemA = $('.menu.main-menu li a'); menuItemA.each(function () { let $this = $(this), text = $this.text(); $this.attr('data-hover', text); let datHov = $this.data('hover'); datHov.replace(/\s/g, "&nbsp;"); })
            menuItemA.on('mouseenter', function (e) {
                let $this = $(this), parentLi = $this.parent('li'), miPosTop = parentLi.position().top; mainMenu.addClass('hovered')
            ...
            menuItemA.on('mouseleave', function (e) {
                let $this = $(this); menuItemA.removeClass('hovered')
            ...
            menuToggle.on('click', function () {
                siteNav.removeClass('open'); var clicks = $(this).data('clicks'); var $this = $(this); if (clicks) {
                    if ($('.sub-menu').hasClass('opened')) {
                        $('ul.opened > li > a').addClass('cakomako')
                        gsap.fromTo('ul.opened > li > a', { translateY: "0%" }, {
                            translateY: "100%", overwrite: true, stagger: -0.05, ease: "power2.in", onStart: function () { $('.sub-back').removeClass('is-active') }, onComplete: function () {
                                siteNav.removeClass('menu-opened'); headerWrapper.removeClass('menu-opened'); menuToggle.removeClass('is-active'); enableScroll(); $('.site-header').removeClass('menu-has-open'); $('.site-navigation ul').removeClass('hidden')
                                $('.site-navigation ul').removeClass('opened');
                            }
                        })
                    } else { menuAin.reverse(); }
                    socialListAnim.reverse(); gitButtonAnim.reverse();
                } else {
                    ...
                    var rule = CSSRulePlugin.getRule(".site-header.fullscreen_menu.menu-has-open::before"); if (mobileQuery.matches) { gsap.set(rule, { cssRule: { height: '100vh' } }); } else {
    ...
    function aliothBlog() {
        var blog = $('.alioth-blog'), post = blog.find('.post'), imagesWrap = blog.children('.post-images'), imagesImg, findImg; if (blog.hasClass('blog-list')) {
            post.each(function (i) {
                ...
                $this.on('mouseleave', function () { let imageCheck = $this.data('post'); findImg = '.' + imageCheck; gsap.fromTo(findImg, { width: '100%' }, { width: "0%", }) })
            }); imagesImg = imagesWrap.find('.post-image'); blog.on('mousemove', function (e) { gsap.to(imagesWrap, { left: e.pageX, top: e.pageY - $(window).scrollTop(), duration: .6 }) })
    ...
    function aliothProjectPage() {
        const ALL_PROJECTS = [
            {
                ...
                img: '../img/projects/summit-collective/summit-featured.mp4',
                year: '2026'
            },
            {
                slug: 'fleur',
                title: 'fleur.',
                category: 'web-development-and-design',
                img: '../img/projects/fleur/fleur-featured.mp4',
                year: '2026'
            },
            {
                slug: 'abe-group',
            ...
            },
            {
                ...
                category: 'web-development-and-design',
                img: '../img/projects/caliz-music-studio/caliz-featured.mp4',
                year: '2022'
            },
            {
            ...
            },
            {
                ...
                title: 'Wreck-It HAU',
                category: 'graphic-and-motion',
                img: '../img/projects/wreck-it-hau/wreck-it-hau-featured.mp4',
                year: '2020'
            },
            {
                slug: 'kaleidoscope',
            ...
            },
            {
                ...
                title: 'HAUwak Kamay',
                category: 'graphic-and-motion',
                img: '../img/projects/hauwak-kamay/hauwak-kamay-featured.mp4',
                year: '2020'
            },
            {
                slug: 'ivy-league',
                        ...
                        <div class="post alioth-post">
                            <a href="/projects/${proj.slug}">
                                <div class="post-image">
                                    ${proj.img.endsWith('.mp4') ? `<video autoplay loop muted playsinline draggable="false"><source src="${proj.img}" type="video/mp4"></video>` : `<img alt="Post Image" src="${proj.img}">`}
                                </div>
                                <div class="post-meta">
        ...
        let projectHeader = $('.project-page-header'), projectImage = $('.project-featured-image'), projectImg = projectImage.children('img'), animate = projectHeader.data('animate'), title = projectHeader.find('.project-title'), cat = projectHeader.find('.project-cat'), other = projectHeader.find('.project-other h5'), summary = projectHeader.find('.meta-summary h5'), video = projectHeader.find('.project-featured-video'), nextVideo = $('.next-project-video'); if (video.length) {
        ...
        if (animate == true) {
            let titleText = $('.project-title h1'); new SplitText(titleText, { type: 'chars, lines', charsClass: 'tt-char', linesClass: 'tt-line' })
            new SplitText(cat, { type: 'chars', charsClass: 'cat-char' })
            new SplitText(summary, { type: 'lines', linesClass: 'summ_line' })
            $('.tit_word, .project-other h5, .summ_line, .project-cat').wrapInner("<span></span>"); if (projectHeader.hasClass('style_1')) {
                let pphAnim = gsap.timeline(); pphAnim.to('.tt-char', 1.5, { y: '0%', stagger: 0.02, ease: 'power3.out' }, 0)
                pphAnim.to('.cat-char', 1, { y: '0%', stagger: 0.02, ease: 'power3.out' }, .5)
                pphAnim.to('.summ_line span', 1.5, { y: '0%', stagger: 0.1, ease: 'power2.out' }, .5)
                pphAnim.to('.project-other span', 1.5, { y: '0%', stagger: 0.1, ease: 'power2.out' }, .7)
                ...
                pphAnim.to('.project-other span', 1.5, { y: '0%', stagger: 0.1, ease: 'power2.out' }, .5)
    ...
    function aliothWorks() {
        var works = $('.alioth-works'), willAnimate = works.data('animate'), categories = works.find('.aw-categories'), catFilter = categories.find('li'), worksWrap = works.children('.aw-works-wrapper'), project = worksWrap.find('.aw-project'), worksScroll; catFilter.first().addClass('active')
        project.each(function (i) {
            i++
            let $this = $(this), category = $this.data('category'); $this.addClass('cat_' + category)
            $this.wrapInner('<div class="aw-project-wrap"></div>')
        }); var $grid = $('.aw-works-wrapper').masonry({ itemSelector: '.aw-project', percentPosition: false, columnWidth: '.aw-works-sizer', initLayout: false, gutter: ".aw-works-gutter", stamp: ".aw-works-stamp", transitionDuration: 0 }); $grid.imagesLoaded().progress(function () { $grid.masonry(); }); $grid.masonry('once', 'layoutComplete', function () {
            project.each(function () {
                let $this = $(this), width = $this.outerWidth(), awpWrap = $this.find('.aw-project-wrap'), awpA = $this.children('a'), awpImage = awpWrap.find('.aw-project-image'); gsap.set(awpWrap, { width: width })
        ...
        catFilter.on('click', function () {
            let $this = $(this), cat = $this.data('cat'), projectsFind = '.cat_' + cat, appPro = $(projectsFind); if (!$this.hasClass('active')) {
    ...
    function aliothShoppingCart() {
        if ($('.cart-page').length) {
            let productQuant = $('.cpq-number'), decrase = productQuant.children('.cpq-reduce'), increase = productQuant.children('.cpq-increase'), totQuant = productQuant.children('.cpq-num'); var clicks = 0; increase.on('click', function () {
                clicks++
    ...
    function aliothProductsCarousel() {
        var apCarousel = $('.alioth-products-carousel'); apCarousel.each(function () {
            let $this = $(this), apWrapper = $this.find('.apc-product-wrapper'), cats = $this.find('.apc-cats ul'), cat = cats.find('li'), products = $this.find('.apc-product'), willAnim = $this.data('anim'), pressedTop; products.append('<span class="product-ov"></span>')
            InertiaPlugin.track(apWrapper, "x"); var apcDrag = Draggable.create(apWrapper, { type: "x", bounds: $this, autoScroll: true, inertia: true, edgeResistance: 0.75, dragResistance: 0.4, throwProps: true, }); if (willAnim == true) {
                gsap.fromTo(apWrapper, { x: -5100 }, { x: '0%', duration: 4, ease: 'power2.inOut', scrollTrigger: { trigger: $this, } })
                gsap.fromTo(cat, { y: '100%' }, { y: '0%', stagger: 0.2, delay: 1.5, duration: .8, ease: 'power1.out', scrollTrigger: { trigger: $this, } })
            }
            cat.on('click', function () {
                let $this = $(this), dataCat = '.cat_' + $this.data('category'), filterAn = gsap.timeline(); cat.removeClass('active'); $this.addClass('active'); filterAn.fromTo('.product-ov', { width: '0%' }, { width: '100%', ease: 'power2.inOut', onProgress: function () { cats.addClass('locked') }, onStart: function () { gsap.set('.product-ov', { right: 0, left: 'unset' }) }, onComplete: function () { products.addClass('hide'); $(dataCat).removeClass('hide'); let draggable = Draggable.get(apWrapper); draggable.update(true); } })
    ...
    function aliothAwards() {
        var awards = $('.alioth-awards1')
        awards.each(function () {
            let $this = $(this), award = $this.find('.a-award1'), willAnim = $this.data(''); if (willAnim == true) {
                award.each(function () {
                    let $this = $(this), title = $this.find('.award-title1'), loc = $this.find('.award-loc1'), date = $this.find('.award-date1'); title.wrapInner('<span></span>')
                    loc.wrapInner('<span></span>')
                    date.wrapInner('<span></span>')
                    let spans = $this.find('span'); gsap.fromTo(spans, { y: '100%' }, { y: '0%', duration: .75, stagger: 0.1, ease: 'power2.Out', scrollTrigger: { trigger: $this, onEnter: function () { $this.addClass('is_inview') }, } })
                })
            }
        });
    }; function aliothSingleProduct() {
        if ($('.product-page').length) {
            let firstImage = $('.sp-image:first-child > img'), firstHeight = firstImage.outerHeight(), slider = $('.sp-slider'); gsap.set(slider, { height: firstHeight })
            ...
            metaButtons.on('click', function () {
                let $this = $(this), parent = $this.parent('li'), desc = parent.children('.desc'), close = parent.children('.desc-close')
    ...
    function aliothScrollNotice() {
        var scrollNot = $('.scroll-notice'); if (scrollNot.length) {
            var scLine = scrollNot.find('.sn_bef'), scTim = gsap.timeline({ repeat: -1 }); scTim.fromTo(scLine, { height: 40, }, { height: 0, duration: 1, onStart: function () { gsap.set(scLine, { top: 'unset', bottom: '140%' }) } })
    ...
    function showcaseFullscreenCarousel() {
        var aServSlider1 = new Swiper('.fullscreen-carousel-showcase', { slidesPerView: 3, navigation: { nextEl: '.slide-next', prevEl: '.slide-prev', }, mousewheel: { invert: false } }); let projects = $('.cs-project'); projects.each(function (i) {
            i++
            ...
            $('.cs-project-image').hide(); let imgCheck = $this.data('image'), findImg = '.image_' + imgCheck; $this.on('mouseenter', function () {
                $this.addClass('hovered'); gsap.fromTo(findImg, { scale: 1.1, opacity: 0 }, {
                    scale: 1, opacity: 1, onStart: function () {
                        $(findImg).show(); gsap.set('.cs-project-image', { zIndex: -1 })
                        gsap.set(findImg, { zIndex: 1 })
                    },
                })
            }); $this.on('mouseleave', function () { $this.removeClass('hovered'); gsap.to(findImg, { opacity: 0, delay: .3, onComplete: function () { $(findImg).hide(); } }) })
        })
    }
    function showcaseFullscreenWall() {
        var projects = $('.fw-projects'), project = projects.find('.fw-project'); project.each(function (i) {
            i++
            let $this = $(this), imageURL = $this.data('image-url'), videoID = $this.data('plyr-embed-id'); if (imageURL != null) {
        ...
        project.on('mouseenter', function () {
            let $this = $(this), findImage = $this.data('image'), imgWrap = $(findImage).find('.fw-project-image-wrap'), img = imgWrap.find('img'), cat = $this.find('.fw-project-category').text(); $('.fw-cat').html('<span>' + cat + '</span>'); new SplitText('.fw-cat', { type: 'chars', charsClass: 'fw-cat-char' })
        ...
        project.on('mouseleave', function () {
            let $this = $(this), findImage = $this.data('image'), imgWrap = $(findImage).children('.fw-project-image-wrap'), img = imgWrap.children('img'); $this.removeClass('active')
    ...
    function showcaseWall() {
        var wallProject = $('.wall-project'), wallParent = $('.wall-projects'), parentSec = wallParent.parents('.section'), wallProjectsTop = document.getElementsByClassName('wall-projects-top')[0], wallProjectsBottom = document.getElementsByClassName('wall-projects-bottom')[0], topWidth = wallProjectsTop.offsetWidth, bottomWidth = wallProjectsBottom.offsetWidth, winWidth = window.outerWidth, topTrans = topWidth - winWidth, bottomTrans = bottomWidth - winWidth; wallProject.each(function (i) {
            i++
            let $this = $(this); $this.attr('data-image', '.image-' + i)
            $this.find('.project-title').attr('data-index', '0' + i)
            let wallImage = $this.children('.project-image').find('img').attr('src'), wallVideo = $this.find('.showcase-video'), wallImages = $('.wall-images'); if (wallImage != null) { wallImages.append('<div class="wall-image-fix image-' + i + '"><img src="' + wallImage + '"></div>'); } else { wallVideo.appendTo(wallImages).wrap('<div class="wall-image-fix image-' + i + '"></div>') }
        }); var wallImages = $('.wall-image-fix'), wallImagesWidth = wallImages.outerWidth(), wallImg = wallImages.find('img, .plyr'); gsap.set(wallImg, { width: wallImagesWidth })
        wallProject.on('mouseenter', function () {
            let $this = $(this); wallParent.addClass('on-hover'); wallProject.removeClass('hovered'); $this.addClass('hovered'); let findImage = $this.data('image'), findImg = $(findImage).children('img, .plyr'); gsap.fromTo(findImg, { scale: 1.5, rotate: 15 }, { scale: 1, rotate: 0, overwrite: true, duration: .7, ease: 'power2.Out', })
    ...
    function alitohServicesS1() {
        var services = $('.alioth-services.style_1'), service = services.find('.service'), parentSec = services.parents('.section'), serLength = service.length; if (services.length) {
            service.each(function () {
                let $this = $(this), title = $this.find('.service-title'), content = $this.find('.service-content'); new SplitText(title, { type: 'lines, chars', linesClass: 'tit-line', charsClass: 'tit-char' })
                new SplitText(content, { type: 'lines', linesClass: 'cont-line' })
            }); var ssss = new Swiper('.service-images', { slidesPerView: 1, direction: 'vertical', freeMode: true, freeModeMinimumVelocity: 0.99, mousewheel: { invert: false, eventsTarget: '.alioth-services', freeModeSticky: true, sensitivity: 0.3 }, }); let findSer; findSer = $('.s-imgo.swiper-slide-active').data('service'); $(findSer).addClass('active'); ssss.on('activeIndexChange', function () {
                let titleChars
    ...
    function aliothServicesS2() {
        if ($('.alioth-services').length) {
            let aliothSer2 = $('.alioth-services.style_2'), service = aliothSer2.find('.service'), willanim = aliothSer2.data('anim'); service.each(function (i) {
                i++
                ...
                if (willanim == true) {
                    new SplitText(title, { type: 'lines, chars', charsClass: 'ser_tit_char', linesClass: 'ser_tit_line' })
            ...
            service.find('.post-title, .service-toggle').on('click', function (e) {
                e.stopPropagation(); let parent = $(this).closest('.service'), content = parent.children('.service-wrap'), contentInner = content.children('.service-cont'), contHeight = parent.data('height'); if (parent.hasClass('active')) {
                    parent.removeClass('active'); gsap.to(content, 0.6, { height: 0, ease: 'power1.out', delay: 0.1 })
                    gsap.to(contentInner, 0.8, { opacity: 0, ease: 'power1.out' })
                } else {
    ...
    function aliothPersonalHead() {
        let aph = $('.alioth-personal-head'); aph.each(function () {
            let $this = $(this), name = $this.children('.aph-name'), image = $this.children('.aph-image'), willAnim = $this.data('animate'); name.clone().addClass('back').insertAfter(name); if (willAnim == true) {
                ...
                aphWelcome.fromTo('.name_char', 1.5, { y: '100%' }, {
                    y: '0%', stagger: 0.03, ease: 'power2.out', onComplete: function () {
                        const mobileQuery = window.matchMedia('(max-width: 450px)')
                        if (!mobileQuery.matches) {
                            $this.on('mousemove', function (e) {
                                let mouseLeft = e.pageX, mouseTop = e.pageY, names = $this.find('.aph-name')
                                gsap.to(names, { x: -mouseLeft / 10, duration: .6, }); gsap.to('.aph-image', { x: -mouseLeft / 20, duration: .6, });
                            })
                        }
                    }
                }, .5)
                ...
                aphWelcome.fromTo('.circular-button', 1, { width: 0, height: 0 }, { width: 150, height: 150, ease: 'power2.inOut', onComplete: function () { gsap.to('.circular-button span', { opacity: 1 }) } }, 1.5)
            } else {
                $this.on('mousemove', function (e) {
                    let mouseLeft = e.pageX, mouseTop = e.pageY, names = $this.find('.aph-name')
    ...
    function aliothForms() {
        let form = $('form'); form.each(function () {
            let $this = $(this), input = $this.find(":input"); input.on('focus', function () { let $this = $(this), fieldWrap = $this.parent('div'); fieldWrap.addClass('focus') })
    ...
    function alitohNumberCt() {
        var aCount = $('.a-number-counter'); aCount.each(function () {
            let $this = $(this), acNumber = $this.children('.ac-number'), sign = $this.children('.ac-sign'), acTitle = $this.children('.ac-title'); acTitle.wrapInner('<span></span>')
            sign.wrapInner('<span></span>'); let signSpan = sign.children('span'); gsap.set(signSpan, { y: '100%', display: 'block' })
            acNumber.each(function () {
                let $this = $(this), countParent = $this.parent(aCount); let numVal = $this.text(), num1 = "<span class='num_val_anim'>" + (numVal - 3) + "</span>", num2 = "<span class='num_val_anim'>" + (numVal - 2) + "</span>", num3 = "<span class='num_val_anim'>" + (numVal - 1) + "</span>"; $this.prepend(num1, num2, num3); $this.wrapInner("<div class='numbers-wrapper'></div>"); var numWrapper = $this.children('.numbers-wrapper'), parent = $this.parents(aCount), delay = parent.data('delay'), nums = $this.find('.num_val_anim'); gsap.to(numWrapper, 1.5, {
                    y: "-75%", delay: delay, ease: "power2.inOut", scrollTrigger: { trigger: parent, position: "bottom bottom" }, onStart: function () { countParent.addClass('count_inview') }, onComplete: function () {
    ...
    function aliothSingleImage() {
        var singleImage = $('.single-image'); singleImage.each(function () {
            let $this = $(this), img = $this.children('img'), imgSrc = $this.find('img').attr('src'), imgParallax = $this.attr('data-parallax'), parallaxType = $this.attr('data-parallax-type'), lightBox = $this.attr('data-lightbox'); var mobileQuery = window.matchMedia('(max-width: 900px)')
            if (!mobileQuery.matches) {
                if (imgParallax === 'true') {
                    if (parallaxType === 'zoom') {
                        let finalHeight = img.outerHeight() - 200; $this.addClass('parallax_wrapper'); gsap.set($this, { height: finalHeight }); gsap.set(img, { y: -100 })
                        gsap.to(img, { scale: 1.25, scrollTrigger: { trigger: $this, start: "top bottom", scrub: 1.25, end: "bottom top" } })
                    } else if (parallaxType === 'directional') {
                        let finalHeight = img.outerHeight() - 200; img.hide(); $this.addClass('parallax_wrapper')
    ...
    function alitohImageCarousel() {
        var imageCarousel = $('.a-image-carousel'); imageCarousel.each(function (i) {
            i++
    ...
    function aliothPageNav() {
        $('.alioth-page-nav').each(function () {
            let $this = $(this), title = $this.find('.page-title'), text = title.text(); title.append('&nbsp;' + text + '&nbsp;')
            title.append('&nbsp;' + text + '&nbsp;')
    ...
    function aliothTestimonials() {
        var testimonials = $('.a-testimonials'), activeTest, activeIndex, nextTest, prevTest, lastTest, firstTest, testSpans; testimonials.each(function () {
            let $this = $(this), testimonial = $this.find('.a-testimonial'), controls = $this.children('.a-testimonials-control'), wrapper = $this.children('.a-testimonials-wrapper'), prev = $this.find('.a-test-prev'), next = $this.find('.a-test-next'), total = testimonial.length; testimonial.first().addClass('active'); $('.a-test-total').html('0' + total)
            ...
            function checkTestimonals() {
                activeTest = $('.a-testimonial.active'); activeIndex = activeTest.data('testimonial'); nextTest = activeTest.next('.a-testimonial'); prevTest = activeTest.prev('.a-testimonial'); lastTest = $('.a-testimonial').last(); firstTest = $('.a-testimonial').first(); testSpans = activeTest.find('span'); if (!prevTest.length) { prevTest = lastTest; }
            ...
            next.on('click', function (i) {
                checkTestimonals(); gsap.to(wrapper, { height: nextTest.outerHeight() })
                gsap.fromTo(testSpans, 0.75, { y: '0%' }, {
                    y: '-110%', stagger: 0.1, ease: 'power2.Out', onComplete: function () {
                        activeTest.removeClass('active')
                        nextTest.addClass('active'); checkTestimonals(); gsap.fromTo(testSpans, .6, { y: '110%' }, { y: '0%', stagger: 0.05, ease: 'power2.Out', })
                    }
                })
            }); prev.on('click', function (i) {
                checkTestimonals(); gsap.to(wrapper, { height: prevTest.outerHeight() })
                gsap.fromTo(testSpans, 0.75, { y: '0%' }, {
                    y: '-110%', ease: 'power2.In', stagger: 0.1, onComplete: function () {
                        activeTest.removeClass('active')
                        prevTest.addClass('active'); checkTestimonals(); gsap.fromTo(testSpans, .6, { y: '110%' }, { y: '0%', stagger: 0.05, ease: 'power2.Out', })
                    }
                })
            }); if ($this.hasClass('autoplay')) {
                let testCounter = testimonials.find('.a-testimonials-count'), testProgress = testCounter.children('span'); let testAutPlay = gsap.fromTo(testProgress, { width: '0%' }, { width: '100%', duration: 5, repeat: -1, ease: 'none', scrollTrigger: { trigger: testimonials, start: 'top bottom', }, onRepeat: function () { next.trigger('click') } })
    ...
    function aliothSeperator() {
        let seperator = $('.alioth-seperator'); seperator.each(function () {
            let $this = $(this), bgColor = $this.data('color'), willAnim = $this.data('anim'); gsap.set($this, { backgroundColor: bgColor })
            if (willAnim == true) { gsap.to($this, 1.5, { width: '100%', scrollTrigger: { trigger: $this }, ease: 'power1.inOut' }) }
        });
    }
    function aliothButtons() {
        var aButton = $('.a-button'); aButton.each(function () {
            var $this = $(this), overlay = '<span class="button-overlay"></span>'; if ($this.hasClass('style_1')) {
                $this.prepend(overlay); let parentOffset = $this.offset(), overlayIn = $this.children('.button-overlay')
                $this.on('mouseenter', function (e) {
                    gsap.set(overlayIn, { left: e.pageX - parentOffset.left, top: e.pageY - parentOffset.top })
                    gsap.to(overlayIn, { width: '100%', height: '100%' })
                }); $this.on('mouseleave', function (e) { gsap.to(overlayIn, { width: '0%', height: '0%' }) })
            }
        }); var circularButton = $('.circular-button'); circularButton.each(function () { let $this = $(this), target = $this.attr('href'); if ($this.hasClass('scroller')) { $this.on('click', function (e) { e.preventDefault(); gsap.to(window, { duration: 1, scrollTo: target, ease: 'power2.out' }); }) } })
    ...
    function aliothEmbedVideo() {
        if ($('.alioth-embed-video').length > 0) {
            var aEmbedVideo = $('.alioth-embed-video'); aEmbedVideo.each(function (i) {
                i++
    ...
    function aliothHeading() {
        var heading = $('.alioth-heading'); heading.each(function () {
            let $this = $(this), parallax = $this.data('parallax'), image = $this.data('image'), bgText = $this.data('background-text'), img = $this.children('.ah-image'), title = $this.children('.ah-title'); if (parallax == true) {
                $this.addClass('will_anim')
                if (image == true) {
                    $this.addClass('with_image'); gsap.to(img, { y: -100, scrollTrigger: { trigger: $this, start: 'top bottom', scrub: true } })
                    gsap.to(title, { y: 100, scrollTrigger: { trigger: $this, start: 'top bottom', scrub: true } })
                } else {
                    $this.addClass('no-image'); if (bgText != null) {
    ...
    function aliothRecentWorks() {
        var recentWorkCarousel = $('.a-recent-works'); recentWorkCarousel.each(function () {
            let $this = $(this), wrapper = $this.children('.recent-works-wrapper'), wrapperWidth = wrapper.outerWidth(), wrapTransVal = wrapperWidth - window.outerWidth + window.outerWidth / 2, bgText = $this.find('.recent-works-bg-text'), parentSec = $this.parents('.wrapper'), navType = $this.data('navigate'); if (navType === 'scroll') {
                ...
                gsap.fromTo($this, { x: '0%' }, { x: '-25%', scrollTrigger: { trigger: parentSec, pin: false, scrub: 2, start: 'bottom bottom', end: 'bottom top', } })
            } else if (navType === 'arrows') {
                $this.addClass('navby-arrows'); let slides = $this.find('.ar-work'), totSlides = slides.length, slideWidth = $('.ar-work').outerWidth(), nextButton = $('.arw-next'), prevButton = $('.arw-prev'); slides.each(function (i) {
                    i++
    ...
    function aliothLinkedText() {
        var linkedText = $('.linked-text'); linkedText.each(function () {
            let $this = $(this), text = $this.find(">:first-child"), links = $this.find('a'), height = text.css('font-size'), finalHeight = parseInt(height, 10) + 24; new SplitText(text, { type: 'lines, words', linesClass: 'linked-line', wordsClass: 'linked-word' })
            ...
            $(window).on('resize', function () {
                let text = $this.find(">:first-child"), links = $this.find('a'), height = text.css('font-size'), finalHeight = parseInt(height, 10) + 24; gsap.set($this.find('.linked-line'), { height: finalHeight })
                gsap.set($this.children(), { lineHeight: finalHeight + 'px' })
            })
        });
    }
    function aliothTeamCarousel() {
        var aliothTeam = $('.alioth-team'); aliothTeam.each(function () {
            let $this = $(this), container = $this.children('.team-container'), member = $this.find('.swiper-slide'); container.addClass('cakomako')
            var teamCarousel = new Swiper('.team-container', { slidesPerView: "auto", navigation: { nextEl: '.prev' }, spaceBetween: 15, pagination: { el: '.team-progress', type: 'progressbar', }, grabCursor: true, }); member.on('mouseenter', function () {
                let $this = $(this), socials = $this.children('.tm-socials'), icons = socials.find('li'); var socialsAnim = gsap.to(icons, .4, { x: 0, opacity: 1, stagger: 0.1, overwrite: true })
                window.socialsAnim = socialsAnim;
            }); member.on('mouseleave', function () { let $this = $(this), socials = $this.children('.tm-socials'), icons = socials.find('li'); var socialsAnim = gsap.to(icons, .4, { x: '100%', opacity: 0, stagger: -0.1, overwrite: true }) })
        })
    }
    ...
    function aliothShowcaseList() {
        var showcaseList = $('.showcase-list'), wrapper = $('.showcase-list-wrapper'), listProject = showcaseList.find('.sl-project'), slImages = $('.sl-images'), activeImage; listProject.each(function (i) {
            i++
    ...
    function aliothShowcaseFullscrenSlider() {
        var project = $('.fs-project'), fsImages = $('.fs-images'), fract = $('.fs-fraction'), currentSlide, nextSlide, prevSlide, titLines, mets, but, fsTit, actIndex, actImg, nextImg, prevImg, activeProj; project.each(function (i) {
            i++
        ...
        fsSlider.on('slideNextTransitionStart', function () {
            if (!mobileQuery.matches) {
                gsap.fromTo(actImg, 2, { scale: 1, rotate: 0 }, { scale: 1.1, rotate: -5 }, 0)
    ...
    function aliothShowcaseCarousel() {
        var project = $('.cas-project'), wrapper = $('.cas-project-wrapper'), projectTitles = $('.cas-titles'), headline = $('.cas-headline'), bgText = $('.cas-bg-text'), wrapFirstTrans = $(window).outerWidth() / 100 * 90, activeProject; new SplitText(headline, { type: 'lines', linesClass: 'cas-line', })
        project.each(function (i) {
            i++
            ...
            projectTitles.append(title.addClass('title_' + i)); title.attr('data-project', '.cas_project_' + i)
        ...
        let csw = gsap.fromTo(wrapper, { x: wrapFirstTrans }, { x: '-' + (wrapper.outerWidth() - $(window).outerWidth() + 350) }), windowWidth = $(window).outerWidth(), css = new ScrollTrigger({
            trigger: '.carousel-showcase', animation: csw, pin: true, scrub: 1, id: 'showcaseScroll', start: 'top top', end: 'bottom+=3000 top', markers: false, onUpdate: function (self, progress) {
                let prog = $('.cas-progress span'); gsap.to(prog, { width: self.progress * 100 + '%' })
                project.each(function () { let $this = $(this) })
            }, onLeave: function () { gsap.to('.showcase-footer', { opacity: 0 }) }, onEnterBack: function () { gsap.to('.showcase-footer', { opacity: 1 }) },
        }); $('.cs-title').on('mouseenter', function () { let $this = $(this); $this.addClass('active') })
    ...
    function aliothShowcaseSlide() {
        ...
        var prjectsSlider = new Swiper('.ss1-images', {
            slidesPerView: 1, speed: 1000, navigation: { nextEl: '.ss1-next', prevEl: '.ss1-prev', }, pagination: { el: '.ss1-dots', type: 'bullets', clickable: true, renderBullet: function (index, className) { return '<span class="' + className + '">0' + (index + 1) + '</span>'; } }, mousewheel: { invert: false, eventsTarget: '.showcase-slideshow' }, loop: true, direction: 'vertical', on: {
                progress: function (swiper, progress) { }, slideChange: function () { slideCheck(); }, slidePrevTransitionStart: function () {
                    let slidePrevOut = gsap.timeline({ yoyo: true }); slidePrevOut.fromTo(prevImage, 1.5, { rotate: -5, scale: 1.2 }, { rotate: 0, scale: 1, ease: 'power2.out' }, 0)
                    ...
                    lines.each(function () { slidePrevOut.fromTo(chars, .6, { y: '0%' }, { y: '110%', ease: 'power2.in', stagger: -.01 }, 0) })
                }, slidePrevTransitionEnd: function () {
                    slideCheck(); let slidePrevIn = gsap.timeline({ yoyo: true }); slidePrevIn.fromTo(catChars, .5, { y: '-100%', }, { y: '0%', ease: 'power2.out', stagger: 0.01 }, 0)
                    slidePrevIn.fromTo(sumLines, .6, { y: "-100%" }, { y: "0%", stagger: -.02, ease: 'power2.out' }, .3)
                    lines.each(function () { slidePrevIn.fromTo(chars, .6, { y: '-110%' }, { y: '0%', ease: 'power2.out', stagger: -.01 }, 0) })
                }, slideNextTransitionStart: function () {
                    let slideNextOut = gsap.timeline({ yoyo: true }); slideNextOut.fromTo(nextImage, 1.5, { rotate: 5, scale: 1.2 }, { rotate: 0, scale: 1, ease: 'power2.out' }, 0)
                    ...
                    lines.each(function () { slideNextOut.fromTo(chars, .6, { y: '0%', }, { y: '-110%', ease: 'power2.in', stagger: .01 }, 0) })
                }, slideNextTransitionEnd: function () {
                    slideCheck(); let slideNextIn = gsap.timeline({ yoyo: true }); slideNextIn.fromTo(catChars, .5, { y: '100%', }, { y: '0%', ease: 'power2.out', stagger: -0.01 }, 0)
                    slideNextIn.fromTo(sumLines, .6, { y: "100%" }, { y: "0%", stagger: .02, ease: 'power2.out' }, .3)
                    lines.each(function () { slideNextIn.fromTo(chars, .6, { y: '110%' }, { y: '0%', opacity: 1, ease: 'power2.out', stagger: .01 }, 0) })
                }
            }
        }); slideCheck();
    }
    function aliothShowcaseSlideV2() {
        var showcaseSlideV2 = $('.showcase-slideshow-v2'), ss2Project = showcaseSlideV2.find('.ss2-project'), totSlides = ss2Project.length, dots = $('.ss2-dot'), backText = $('.ss2-back-text'), totSlides = ss2Project.length, activeURL, currentSlide, titleChar, excerptLine, catSpan, backChars, activeIndex, activeDets; $('.ss2-tot').text('0' + totSlides)
        ...
        ss2Project.each(function (i) {
            ...
            lines.wrapInner('<span></span>'); cat.wrapInner('<span></span>'); new SplitText(title, { type: 'chars, lines', linesClass: 'title-line', charsClass: 'title-char' });
        ...
        ss2Images.wrapInner('<div class="swiper-wrapper"></div>'); ss2Image.each(function (i) {
            i++
        ...
        function slideChecko() {
            currentSlide = $('.swiper-slide-active'); activeDets = currentSlide.data('slide'); titleChar = $(activeDets).find('.title-char'); excerptLine = $(activeDets).find('.excerpt-line span'); catSpan = $(activeDets).find('.ss2-project-cat span'); backText = '.back_' + $(activeDets).data('index'); backChars = $(backText).find('.bt-char span')
            activeURL = $(activeDets).find('a').attr('href'); activeIndex = $(activeDets).data('index');
        }
        slideChecko(); $('.ss2-button a').attr('href', activeURL); $('.ss2-curr').text('0' + activeIndex); $(activeDets).addClass('active'); $(backText).addClass('active'); ss2ImagesSlider.on('slideChange', function () {
            slideChecko(); let slideOut = gsap.timeline({
                yoyo: true, onComplete: function () {
        ...
        ss2ImagesSlider.on('slideChangeTransitionEnd', function () {
            slideChecko(); $('.ss2-button a').attr('href', activeURL); $('.ss2-curr').text('0' + activeIndex); let slideIn = gsap.timeline({ yoyo: true, onStart: function () { $(activeDets).addClass('active'); $(backText).addClass('active'); } })
    ...
    function aliothParallaxScroll() {
        let hasParallax = $('.has-parallax'), parallaxBg = $('.parallax-bg'); hasParallax.each(function () {
            let $this = $(this), strength = $this.data('parallax-strength'), direction = $this.data('parallax-direction'), pStrength = strength * 100 + '%', pVal; if (direction === 'up') { pVal = '-' + pStrength; } else if (direction === 'down') { pVal = pStrength; }
    ...
    function aliothScrollAnimations() {
        var hasAnim = $('.has-anim'); hasAnim.each(function () {
            ...
            let $this = $(this), anim = $this.data('animation'), delay = $this.data('delay'), duration = $this.data('duration'), ovColor = $this.data('color'); if ((anim === 'blockUp') || (anim === 'blockLeft') || (anim === 'blockRight')) {
                ...
                let imgHeight = $this.outerHeight(), imgWidth = $this.outerWidth(); $this.wrap('<div class="img-anim-wrapper"></div>'); let parWrap = $this.parent('.img-anim-wrapper'), sImage = parWrap.parent('.single-image'); gsap.set(parWrap, { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0 })
                ...
                if ((anim === 'slideUp')) {
                    ...
                    gsap.to(parWrap, { height: imgHeight, duration: duration, scrollTrigger: { trigger: parWrap, start: 'top 85%', }, delay: delay, ease: "blockEase" })
    ...
    function showcaseOpenings() {
        var showcaseCheck = $('.portfolio-showcase'); if (showcaseCheck.length > 0) {
            if (showcaseCheck.hasClass('showcase-list')) {
                var scWelcome = gsap.timeline(); scWelcome.fromTo('.sl-project-title', 2, { y: '110%' }, { y: '0%', ease: 'power2.out', stagger: 0.1, }, 0)
                ...
                scWelcome.fromTo('.showcase-footer', .6, { opacity: 0 }, { opacity: 1, }, 2)
            } else if (showcaseCheck.hasClass('showcase-wall')) {
                var wallOpen = gsap.timeline({ once: true, delay: 0, onStart: function () { $('body').addClass('loading') }, onComplete: function () { $('body').removeClass('loading') } })
                ...
                wallOpen.fromTo('.showcase-footer', 1, { opacity: 0 }, { opacity: 1, ease: 'power2.out' }, 3)
            } else if (showcaseCheck.hasClass('showcase-slideshow-v2')) {
                var ss2Welcome = gsap.timeline({ once: true }); ss2Welcome.fromTo('.title-char', 1, { y: '100%' }, { y: '0%', stagger: 0.02, ease: 'power2.out', }, 0)
                ...
                ss2Welcome.fromTo('.showcase-footer', .5, { opacity: 0 }, { opacity: 1, ease: 'power1.out', }, 1)
            } else if (showcaseCheck.hasClass('showcase-slideshow')) {
                let ssWelcome = gsap.timeline({
                    once: true, onStart: function () {
                        disableScroll(); gsap.set('.ss-project.active .ss1-cat', { visibility: 'hidden' })
                        gsap.set('.ss-project.active .ss1-summary', { visibility: 'hidden' })
                    }
                }); let butWidth = $('.ss1-button').outerWidth(); ssWelcome.fromTo('.st-char', 1.5, { y: '110%', }, { y: '0%', stagger: 0.02, ease: 'power2.out', }, 0)
                ...
                ssWelcome.fromTo('.ss1-dots .swiper-pagination-bullet', 1.5, { opacity: 0, x: 50, }, { x: 0, opacity: 1, stagger: 0.05, ease: 'power2.out', onComplete: function () { gsap.to('.ss1-dots .swiper-pagination-bullet', { clearProps: 'opacity' }) } }, 1.55)
            } else if (showcaseCheck.hasClass('carousel-showcase')) {
                let sCarouselWelcome = gsap.timeline({ onStart: function () { disableScroll(); }, onComplete: function () { enableScroll(); } }), wrapper = $('.cas-project-wrapper'), wrapFirstTrans = $(window).outerWidth() / 100 * 90, wrapWidth = -wrapper.outerWidth(); sCarouselWelcome.fromTo('.cas-line span', 1, { y: '100%' }, { y: '0%', stagger: 0.1, ease: 'power3.out' }, 2)
                ...
                sCarouselWelcome.fromTo('.showcase-footer', 1, { opacity: 0, }, { opacity: 1 }, 2.7)
                ...
                welcomeAnim.fromTo('.showcase-footer', 1, { opacity: 0 }, { opacity: 1, ease: 'power2.out', }, 1.7)
            } else if (showcaseCheck.hasClass('fullscreen-wall-showcase')) {
                let fsWallWelcome = gsap.timeline({ once: true }), dashs = CSSRulePlugin.getRule('.fw-project::after'); fsWallWelcome.fromTo('.fw-project a', 1.5, { y: '150%', }, { y: '0%', stagger: 0.1, ease: 'power2.out', }, 0)
    ...
    function initShortcodes() {
        showCaseVideos(); aliothSingleImage(); aliothSingleProject(); alitohImageCarousel(); aliothPageNav(); aliothSeperator(); aliothButtons(); aliothEmbedVideo(); aliothLinkedText(); aliothTeamCarousel(); aliothScrollableText()
    ...
    $(window).on('load', function () {
        initShowcases(); siteHeaderSet(); fullscreenNavigation(); classicNavigation(); if ($('.section.fullscreen').length) { $('#footer').hide() } else { $('#footer').show() }
        pageLoader(); if (siteLoader == true) {
            loadAn.eventCallback('onComplete', function () {
                $('body').removeClass('loading'); gsap.set('#page', { visibility: 'visible' })
            ...
            loader.hide(); if ($('.section.fullscreen').length) { $('#footer').hide() } else { $('#footer').show() }
            showcaseOpenings(); initShortcodes(); initPages(); let mobileQuery = window.matchMedia('(max-width: 900px)'); if (!mobileQuery.matches) { aliothParallaxScroll(); }
            enableScroll()
            aliothScrollAnimations(); ScrollTrigger.refresh(true)
        }
    ...
    barba.init({
        debug: true, cacheIgnore: true, transitions: [{
            name: 'default-transition', leave() {
                return new Promise(function (resolve, reject) {
                    var trans = $('.alioth-page-transitions'), bg = $('.apt-bg'), siteHeader = $('.site-header'); if (siteHeader.hasClass('menu-has-open')) {
                        var fsMenuTrans = gsap.timeline({ onComplete: function () { resolve(); }, }); let subMenu = $('.sub-menu'); if (subMenu.hasClass('opened')) { var menuItems = $('.sub-menu.opened > li > a'); $('.sub-back').removeClass('is-active') } else { var menuItems = $('.main-menu > li > a'); }
                        ...
                        fsMenuTrans.fromTo('.trans_char', .75, { y: "100%" }, {
                            ease: 'power2.out', y: '0%', stagger: 0.01, onStart: function () {
                                if ($('.alioth-page-transitions').hasClass('dark')) { var firstColor = 'hsla(0, 0%, 100%, .2)', secondColor = '#fff'; } else { var firstColor = 'rgba(25,27,29,.6)', secondColor = '#000'; }
                                let tl = gsap.timeline({ once: true, delay: .5 }); tl.to('.trans_char', { color: secondColor, duration: .3, stagger: .02, ease: 'none' })
                                tl.to('.trans_char', { color: firstColor, duration: .3, stagger: .02, ease: 'none' })
                            }, onComplete: function () {
                                gsap.to(siteHeader, 1, { height: 150, ease: 'power2.inOut' })
                                gsap.to('.header-wrapper', 1, { top: '60%', ease: 'power2.inOut' })
                            }
                        }, 1)
                        fsMenuTrans.to('.git-button', 1, { y: '-100%', opacity: 0, stagger: 0.05, ease: 'power2.in' }, 0)
                        fsMenuTrans.to(menuOv, .5, { cssRule: { height: '100vh' }, ease: 'none' }, .1)
                    } else {
                        var defaultTransOut = gsap.timeline({ once: 'true', onStart: function () { gsap.set(trans, { visibility: 'visible' }) }, onComplete: function () { resolve(); } })
                        ...
                        defaultTransOut.fromTo('.trans_char', .75, { y: "100%" }, {
                            ease: 'power2.out', y: '0%', stagger: 0.01, onStart: function () {
                                if ($('.alioth-page-transitions').hasClass('dark')) { var firstColor = 'hsla(0, 0%, 100%, .2)', secondColor = '#fff'; } else { var firstColor = 'rgba(25,27,29,.6)', secondColor = '#000'; }
                                let tl = gsap.timeline({ once: true, delay: .5 }); tl.to('.trans_char', { color: secondColor, duration: .3, stagger: .02, ease: 'none' })
                                tl.to('.trans_char', { color: firstColor, duration: .3, stagger: .02, ease: 'none' })
                ...
                return new Promise(function (resolve, reject) {
                    var trans = $('.alioth-page-transitions'), bg = $('.apt-bg'), text = $('.trans-text'); if (siteHeader.hasClass('menu-has-open')) {
                        gsap.fromTo('.trans_char', .75, { y: "0%" }, { ease: 'power2.in', y: '-100%', stagger: 0.01, delay: .35 })
                        gsap.set(menuBackOv, { display: 'none' })
                        gsap.to(menuOv, .75, {
                            delay: 1, cssRule: { height: 0 }, ease: 'power2.inOut', onStart: function () { resolve(); }, onComplete: function () {
                        ...
                        var transOut = gsap.timeline({ delay: 1, onStart: function () { resolve() }, onComplete: function () { gsap.set(trans, { visibility: 'hidden' }) } }); transOut.fromTo(bg, .7, { height: '100%' }, { height: '0%', onStart: function () { gsap.set(bg, { top: 0, bottom: 'unset' }) }, ease: 'power2.inOut', }, .5)
                        transOut.fromTo('.trans_char', .75, { y: "0%" }, { ease: 'power2.in', y: '-100%', stagger: 0.01 }, 0)
                    }
                })
            }
        }, {
            name: 'fs-image-trans', from: { namespace: ['fs-slider'] }, to: { namespace: ['pph2'] }, leave() {
                return new Promise(function (resolve, reject) {
                    ...
                    fsImageTrans.fromTo('.showcase-footer', 1, { opacity: 1 }, { opacity: 0, ease: 'power2.in', onComplete: function () { var project = $('.fs-project-image.swiper-slide-active'), imageURL = project.find('img').attr('src'); $('body').append('<div class="trans-image"><div class="trans-image-wrap"><img src="' + imageURL + '"></div></div>'); gsap.set('.trans-image', { width: '100%', height: '100%' }) } }, .3)
                })
            }, enter() { return new Promise(function (resolve, reject) { gsap.to('.trans-image', { height: '100vh', duration: 1, ease: 'power2.out', onComplete: function () { resolve(); $('.trans-image').remove(); enableScroll(); } }) }) }
        }, {
            name: 'fs-image-trans-half', from: { namespace: ['fs-slider'] }, to: { namespace: ['pph1'] }, leave() {
                return new Promise(function (resolve, reject) {
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    gsap.to('.trans-image', { height: projHeg, duration: 1.5, delay: 1, ease: 'power2.inOut', onStart: function () { gsap.set('#page', { visibility: 'hidden' }) }, onComplete: function () { $('.trans-image').remove(); resolve(); gsap.set('#page', { visibility: 'visible' }); enableScroll(); } })
                })
            }
        }, {
            name: 'sc-image-trans', from: { namespace: ['sc-carousel'] }, to: { namespace: ['pph2'] }, leave() {
                return new Promise(function (resolve, reject) {
                    disableScroll(); var project = $('.cs-title.active').data('project'), img = $(project).find('img'), imgURL = img.attr('src'), ofLeft = $(project).offset().left; new SplitText('.cs-title a', { type: 'chars', charsClass: 'cst_char' })
                    ...
                    scImageTrans.fromTo('.showcase-footer', 1, { opacity: 1 }, {
                        opacity: 0, ease: 'power2.in', onComplete: function () {
                            $('body').append('<div class="trans-image"><div class="trans-image-wrap"><img src="' + imgURL + '"></div></div>'); const mobileQuery = window.matchMedia('(max-width: 450px)')
                            ...
                            let imageHalf = gsap.timeline(); imageHalf.to('.trans-image', 1, { width: '100%', left: 0, ease: 'power2.inOut', height: '100vh', onComplete: function () { resolve() } })
                        }
                    }, .3)
                })
            }, enter() { $('.trans-image').remove(); enableScroll(); }
        }, {
            name: 'sc-image-trans-half', from: { namespace: ['sc-carousel'] }, to: { namespace: ['pph1'] }, leave() {
                return new Promise(function (resolve, reject) {
                    disableScroll(); var project = $('.cs-title.active').data('project'), img = $(project).find('img'), imgURL = img.attr('src'), ofLeft = $(project).offset().left; new SplitText('.cs-title a', { type: 'chars', charsClass: 'cst_char' })
                    var scImageTrans = gsap.timeline(); scImageTrans.fromTo('.cst_char', 1, { y: '0%', }, { y: '-110%', stagger: 0.01, ease: 'power1.in', }, 0)
                    scImageTrans.fromTo('.cas-progress', 1, { width: '50%', }, { width: '0%', ease: 'power1.in', }, 0)
                    scImageTrans.fromTo('.showcase-footer', 1, { opacity: 1 }, {
                        opacity: 0, ease: 'power2.in', onComplete: function () {
                            $('body').append('<div class="trans-image"><div class="trans-image-wrap"><img src="' + imgURL + '"></div></div>'); const mobileQuery = window.matchMedia('(max-width: 450px)')
                            if (mobileQuery.matches) { gsap.set('.trans-image', { width: '80vw', height: '60vh', top: '50%', y: '-50%', left: ofLeft }) } else { gsap.set('.trans-image', { width: '50vw', height: '50vh', top: '50%', y: '-50%', left: ofLeft }) }
                            let imageHalf = gsap.timeline(); imageHalf.to('.trans-image', 1, {
                                width: '100%', left: 0, ease: 'power2.inOut', height: '100%', onComplete: function () {
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    if (mobileQuery.matches) { var projHeg = '65vh' } else { var projHeg = '55vh' }
                    gsap.to('.trans-image', {
                        height: projHeg, duration: 1.5, delay: 1, ease: 'power2.inOut', onComplete: function () {
                            $('.trans-image').remove(); resolve(); gsap.set('#page', { visibility: 'visible' })
                            enableScroll();
                        }
                    })
                })
            ...
            name: 'fss-image-trans', from: { namespace: ['fs-slideshow'] }, to: { namespace: ['pph2'] }, leave() {
                return new Promise(function (resolve, reject) {
                    ...
                    scImageTrans.fromTo('.ss1-dots .swiper-pagination-bullet', 1, { opacity: 1, x: 0, }, {
                        x: -50, opacity: 0, stagger: 0.02, ease: 'power1.in', onComplete: function () {
                            $('body').append('<div class="trans-image"><div class="trans-image-wrap"><img src="' + imgURL + '"></div></div>'); const mobileQuery = window.matchMedia('(max-width: 450px)')
                            ...
                            let imageHalf = gsap.timeline(); imageHalf.to('.trans-image', 1, {
                                height: '100%', width: '100%', left: 0, ease: 'power2.inOut', onComplete: function () {
                                    gsap.set('.trans-image', { y: '0%', top: '0%' })
                                    resolve()
                                }
                            })
            ...
            name: 'fss-image-trans-half', from: { namespace: ['fs-slideshow'] }, to: { namespace: ['pph1'] }, leave() {
                return new Promise(function (resolve, reject) {
                    disableScroll(); var project = $('.ss1-image-wrap.swiper-slide-active'), activeProj = $('.ss-project.active'), activeChars = activeProj.find('.st-char'), catChars = activeProj.find('.cat_char'), sumLines = activeProj.find('.suml-wrap'), img = project.find('img'), imgURL = img.attr('src'); var scImageTrans = gsap.timeline(); scImageTrans.fromTo(activeChars, 1, { y: '0%', }, { y: '-110%', stagger: 0.01, ease: 'power1.in', }, 0)
                    scImageTrans.fromTo(catChars, 1, { y: '0%', }, { y: '-110%', stagger: 0.01, ease: 'power1.in', }, 0)
                    scImageTrans.fromTo(sumLines, 1, { y: '0%', }, { y: '-110%', stagger: 0.01, ease: 'power1.in', }, 0)
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    if (mobileQuery.matches) { var projHeg = '65vh' } else { var projHeg = '55vh' }
                    gsap.to('.trans-image', {
                        height: projHeg, duration: 1.5, delay: .5, ease: 'power2.inOut', onComplete: function () {
                            $('.trans-image').remove(); resolve(); gsap.set('#page', { visibility: 'visible' })
                            enableScroll();
                        }
                    })
            ...
            name: 'fswall-image-trans-half', from: { namespace: ['fs-wall'] }, to: { namespace: ['pph1'] }, leave() {
                return new Promise(function (resolve, reject) {
                    disableScroll(); var project = $('.fw-project.active').data('image'), img = $(project).find('img'), imgURL = img.attr('src'), dashs = CSSRulePlugin.getRule('.fw-project::after'); var scImageTrans = gsap.timeline(); scImageTrans.fromTo('.fw-project a', 1, { y: '0%', }, {
                        y: '-150%', stagger: 0.05, ease: 'power1.in', onStart: function () {
                            $('body').append('<div class="trans-image"><div class="trans-image-wrap"><img src="' + imgURL + '"></div></div>'); $('.fw-images').hide(); const mobileQuery = window.matchMedia('(max-width: 450px)')
                            if (mobileQuery.matches) { gsap.set('.trans-image', { width: '100%', height: '100%', top: '0', right: '0', left: 'unset', zIndex: -1 }) } else { gsap.set('.trans-image', { width: '50%', height: '100%', top: '0', right: '0', left: 'unset', zIndex: -1 }) }
                        }
                    }, 0)
                    scImageTrans.fromTo(dashs, 1, { cssRule: { y: '0%', } }, { cssRule: { y: '-150%', }, stagger: 0.05, ease: 'power1.in', }, .2)
                    scImageTrans.fromTo('.showcase-footer', .5, { opacity: 1 }, {
                        opacity: 0, ease: 'power1.in', onComplete: function () {
                            let imageHalf = gsap.timeline(); imageHalf.to('.trans-image', 1, {
                                width: '100%', ease: 'power2.inOut', onComplete: function () {
                                    gsap.set('.trans-image', { y: '0%', top: '0%' })
                                    resolve()
                                }
                            })
            ...
            name: 'fswall-image-trans', from: { namespace: ['fs-wall'] }, to: { namespace: ['pph2'] }, leave() {
                return new Promise(function (resolve, reject) {
                    disableScroll(); var project = $('.fw-project.active').data('image'), img = $(project).find('img'), imgURL = img.attr('src'), dashs = CSSRulePlugin.getRule('.fw-project::after'); var scImageTrans = gsap.timeline(); scImageTrans.fromTo('.fw-project a', 1, { y: '0%', }, {
                        y: '-150%', stagger: 0.05, ease: 'power1.in', onStart: function () {
                            $('body').append('<div class="trans-image"><div class="trans-image-wrap"><img src="' + imgURL + '"></div></div>'); $('.fw-images').hide(); const mobileQuery = window.matchMedia('(max-width: 450px)')
            ...
            name: 'scwall-image-trans', from: { namespace: ['sc-wall'] }, to: { namespace: ['pph2'] }, leave() {
                return new Promise(function (resolve, reject) {
                    disableScroll(); var project = $('.wall-project.hovered').data('image'), img = $(project).find('img'), imgURL = img.attr('src'); var scImageTrans = gsap.timeline(); scImageTrans.to('.wall-projects-top', 2, {
                        x: '100%', stagger: 0.05, ease: 'power2.inOut', onStart: function () {
                            $('body').addClass('loading'); $('body').append('<div class="trans-image"><div class="trans-image-wrap"><img src="' + imgURL + '"></div></div>'); const mobileQuery = window.matchMedia('(max-width: 450px)')
                            if (mobileQuery.matches) {
                                gsap.set('.trans-image', { width: '84%', height: '70%', top: '50%', left: '50%', y: '-50%', x: '-50%', zIndex: -1 })
                                $('.wall-images').remove();
                            } else {
                                gsap.set('.trans-image', { width: '30%', height: '70%', top: '50%', left: '50%', y: '-50%', x: '-50%', zIndex: -1 })
                                $('.wall-images').remove();
                            }
                        }
                    }, 0)
                    ...
                    scImageTrans.fromTo('.showcase-footer', .5, { opacity: 1 }, {
                        opacity: 0, ease: 'power1.in', onComplete: function () {
                            let imageHalf = gsap.timeline(); imageHalf.to('.trans-image', 1, {
                                width: '100%', height: '100%', ease: 'power2.inOut', onComplete: function () {
                                    gsap.set('.trans-image', { y: '0%', top: '0%', zIndex: 'unset' })
                                    resolve()
                                }
                            })
                        }
            ...
            }, enter() { return new Promise(function (resolve, reject) { gsap.to('.trans-image', { height: '100vh', duration: 1, ease: 'power2.out', onComplete: function () { resolve(); $('.trans-image').remove(); $('body').removeClass('loading'); enableScroll(); } }) }) }
        }, {
            name: 'scwall-image-trans-half', from: { namespace: ['sc-wall'] }, to: { namespace: ['pph1'] }, leave() {
                return new Promise(function (resolve, reject) {
                    disableScroll(); var project = $('.wall-project.hovered').data('image'), img = $(project).find('img'), imgURL = img.attr('src'); var scImageTrans = gsap.timeline(); scImageTrans.to('.wall-projects-top', 2, {
                        x: '100%', stagger: 0.05, ease: 'power2.inOut', onStart: function () {
                            $('body').addClass('loading'); $('body').append('<div class="trans-image"><div class="trans-image-wrap"><img src="' + imgURL + '"></div></div>'); const mobileQuery = window.matchMedia('(max-width: 450px)')
                    ...
                    scImageTrans.fromTo('.showcase-footer', .5, { opacity: 1 }, {
                        opacity: 0, ease: 'power1.in', onComplete: function () {
                            $('body').removeClass('loading'); let imageHalf = gsap.timeline(); imageHalf.to('.trans-image', 1, {
                                width: '100%', height: '100%', ease: 'power2.inOut', onComplete: function () {
                                    gsap.set('.trans-image', { y: '0%', top: '0%', zIndex: 'unset' })
                                    resolve()
                                }
            ...
            name: 'scslideshow-image-trans', from: { namespace: ['sc-slideshow'] }, to: { namespace: ['pph2'] }, leave() {
                return new Promise(function (resolve, reject) {
                    disableScroll(); var project = $('.swiper-slide.swiper-slide-active'), img = project.find('img'), activeProj = $('.ss2-project.active'), titChars = activeProj.find('.title-char'), cat = activeProj.find('.ss2-project-cat span'), summLines = activeProj.find('.excerpt-line span'), imgURL = img.attr('src'); var scImageTrans = gsap.timeline(); scImageTrans.to(titChars, 1, {
                        y: '-100%', stagger: 0.02, ease: 'power2.in', onStart: function () {
                            $('body').append('<div class="trans-image"><div class="trans-image-wrap"><img src="' + imgURL + '"></div></div>'); const mobileQuery = window.matchMedia('(max-width: 450px)')
                            if (mobileQuery.matches) { gsap.set('.trans-image', { width: '100%', height: '100%', top: '0', right: '0', y: '0', zIndex: -1 }) } else { gsap.set('.trans-image', { width: '35%', height: '70%', top: '50%', left: 'unset', right: '8.5%', y: '-50%' }) }
                        }
                    }, 0)
                    ...
                    scImageTrans.fromTo('.showcase-footer', .5, { opacity: 1 }, {
                        opacity: 0, ease: 'power1.in', onComplete: function () {
                            let imageHalf = gsap.timeline(); imageHalf.to('.trans-image', 1, {
                                width: '100%', height: '100%', right: '0%', ease: 'power2.inOut', onComplete: function () {
                                    gsap.set('.trans-image', { y: '0%', top: '0%' })
                                    resolve()
                                }
                            })
            ...
            name: 'scslideshow-image-trans-half', from: { namespace: ['sc-slideshow'] }, to: { namespace: ['pph1'] }, leave() {
                return new Promise(function (resolve, reject) {
                    disableScroll(); var project = $('.swiper-slide.swiper-slide-active'), img = project.find('img'), activeProj = $('.ss2-project.active'), titChars = activeProj.find('.title-char'), cat = activeProj.find('.ss2-project-cat span'), summLines = activeProj.find('.excerpt-line span'), imgURL = img.attr('src'); var scImageTrans = gsap.timeline(); scImageTrans.to(titChars, 1, {
                        y: '-100%', stagger: 0.02, ease: 'power2.in', onStart: function () {
                            $('body').append('<div class="trans-image"><div class="trans-image-wrap"><img src="' + imgURL + '"></div></div>'); const mobileQuery = window.matchMedia('(max-width: 450px)')
            ...
            name: 'sclist-image-trans', from: { namespace: ['sc-list'] }, to: { namespace: ['pph2'] }, leave() {
                return new Promise(function (resolve, reject) {
                    disableScroll(); var project = $('.sl-project-image.active'), ofLeft = $('.sl-images').position().left, ofTop = $('.sl-images').position().top, img = project.find('img'), imgURL = img.attr('src'); var scImageTrans = gsap.timeline(); scImageTrans.to('.sl-project-title', 1, {
                        y: '-110%', ease: 'power2.in', stagger: 0.05, onStart: function () {
                            $('body').append('<div class="trans-image"><div class="trans-image-wrap"><img src="' + imgURL + '"></div></div>'); const mobileQuery = window.matchMedia('(max-width: 450px)')
                    ...
                    scImageTrans.to('.showcase-footer', .4, { opacity: 0, onComplete: function () { let imageHalf = gsap.timeline(); imageHalf.to('.trans-image', 1, { right: '0%', left: 'unset', x: '0%', top: '0%', y: '0%', left: '0%', width: '100%', height: '100%', ease: 'power2.inOut', onComplete: function () { resolve() } }) } }, 0)
                })
            }, enter() { $('.trans-image').remove(); enableScroll(); }
        }, {
            name: 'sclist-image-trans-half', from: { namespace: ['sc-list'] }, to: { namespace: ['pph1'] }, leave() {
                return new Promise(function (resolve, reject) {
                    disableScroll(); var project = $('.sl-project-image.active'), ofLeft = $('.sl-images').position().left, ofTop = $('.sl-images').position().top, img = project.find('img'), imgURL = img.attr('src'); var scImageTrans = gsap.timeline(); scImageTrans.to('.sl-project-title', 1, {
                        y: '-110%', ease: 'power2.in', stagger: 0.05, onStart: function () {
                            $('body').append('<div class="trans-image"><div class="trans-image-wrap"><img src="' + imgURL + '"></div></div>'); const mobileQuery = window.matchMedia('(max-width: 450px)')
            ...
            }, enter() { return new Promise(function (resolve, reject) { gsap.to('.trans-image', { height: '55vh', duration: 1.5, delay: .5, ease: 'power2.inOut', onComplete: function () { $('.trans-image').remove(); resolve(); enableScroll(); } }) }) }
        }, {
            name: 'title-trans', from: { namespace: ['fs-slider', 'fs-slideshow', 'fs-wall', 'sc-wall', 'sc-carousel', 'sc-slideshow', 'sc-list'] }, to: { namespace: ['pph3', 'pph-video'] }, leave() {
                return new Promise(function (resolve, reject) {
                    ...
                    var ppTitleOut = gsap.timeline({ onComplete: function () { resolve(); } }); ppTitleOut.fromTo(bg, .7, { height: '0%' }, {
                        height: '100%', duration: .7, ease: 'power2.inOut', onStart: function () {
                            gsap.set(trans, { visibility: 'visible' })
                            gsap.set(bg, { top: 'unset', bottom: 0 })
                        },
                    }, 0)
                    ppTitleOut.fromTo('.trans-text-char', .75, { y: '100%' }, { y: '0%', ease: 'power2.out', y: '0%', stagger: 0.01, }, .45)
                })
            }, enter() {
                return new Promise(function (resolve, reject) {
                    var ppTitleEnd = gsap.timeline({}); ppTitleEnd.fromTo(bg, .7, { height: '100%' }, { height: '0%', ease: 'power2.inOut', onStart: function () { resolve(); gsap.set(bg, { top: 0, bottom: 'unset' }) }, }, 0)
                    ppTitleEnd.fromTo('.trans-text-char', .75, { y: '0%' }, {
                        y: '-100%', stagger: 0.01, ease: 'power2.in', onComplete: function () {
                            $('.trans-text').html(defTransText); gsap.set(trans, { visibility: 'hidden' })
                            new SplitText('.trans-text', { type: 'chars', charsClass: 'trans_char' })
                        }
                    }, 0)
                })
            }
        }, {
            name: 'title-next-trans', from: { namespace: ['pph1', 'pph2', 'pph3', 'pph-video'] }, to: { namespace: ['pph1', 'pph2', 'pph3', 'pph-video'] }, leave() {
                return new Promise(function (resolve, reject) {
                    ...
                    ppTitleOut.fromTo('.trans-text-char', 1, { y: '100%' }, { y: '0%', stagger: 0.025, ease: 'power2.out' }, .45)
                })
            }, enter() {
                var ppTitleEnd = gsap.timeline({}); ppTitleEnd.fromTo(bg, .7, { height: '100%' }, { height: '0%', ease: 'power2.inOut', onStart: function () { gsap.set(bg, { top: 0, bottom: 'unset' }) }, }, 0)
                ppTitleEnd.fromTo('.trans-text-char', .4, { y: '00%' }, {
                    y: '-100%', stagger: 0.02, ease: 'power2.in', onComplete: function () {
                        $('.trans-text').html(defTransText); new SplitText('.trans-text', { type: 'chars', charsClass: 'trans_char' })
                        gsap.set(trans, { visibility: 'hidden' })
                    }
                }, 0)
            }
