...
var trans=$('.alioth-page-t1ransitions'),transLayout=trans.data('layout'),text=$('.trans-text'),defTransText=text.html(),bg=$('.apt-bg'),menuOv=CSSRulePlugin.getRule('.site-header.fullscreen_menu.menu-has-open::before'),menuBackOv=CSSRulePlugin.getRule('.site-header.fullscreen_menu.menu-has-open::after');trans.addClass(transLayout)
...
fsMenuTrans.fromTo('.trans_char',.75,{y:"100%"},{ease:'power2.out',y:'0%',stagger:0.01,onStart:function(){if($('.alioth-page-t1ransitions').hasClass('dark')){var firstColor='hsla(0, 0%, 100%, .2)',secondColor='#fff';}else{var firstColor='rgba(25,27,29,.6)',secondColor='#000';}
...
defaultTransOut.fromTo('.trans_char',.75,{y:"100%"},{ease:'power2.out',y:'0%',stagger:0.01,onStart:function(){if($('.alioth-page-t1ransitions').hasClass('dark')){var firstColor='hsla(0, 0%, 100%, .2)',secondColor='#fff';}else{var firstColor='rgba(25,27,29,.6)',secondColor='#000';}
let tl=gsap.timeline({once:true,delay:.5});tl.to('.trans_char',{color:secondColor,duration:.3,stagger:.02,ease:'none'})
tl.to('.trans_char',{color:firstColor,duration:.3,stagger:.02,ease:'none'})}},.3)}})},enter(){return new Promise(function(resolve,reject){var trans=$('.alioth-page-t1ransitions'),bg=$('.apt-bg'),text=$('.trans-text');if(siteHeader.hasClass('menu-has-open')){gsap.fromTo('.trans_char',.75,{y:"0%"},{ease:'power2.in',y:'-100%',stagger:0.01,delay:.35})
...
var trans=$('.alioth-page-t1ransitions'),bg=$('.apt-bg'),image=$('.apt-image'),transText=$('.trans-text');transText.html(title)
